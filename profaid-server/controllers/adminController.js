...
const addProfessor = async (req, res) => {
  try {
    const { ProfessorID, Name, Email, Password,PhoneNumber, Department, Subjects } = req.body;

    if (!ProfessorID || !Name || !Email || !Password || !PhoneNumber || !Department) {
...
const updateProfessor = async (req, res) => {
  try {
    const { id } = req.params;
    const { Name, Email,PhoneNumber, Department, Subjects } = req.body;

    const subjectIDs = Array.isArray(Subjects) ? Subjects : [];

    const updated = await Professor.findByIdAndUpdate(
      id,
      { Name, Email,PhoneNumber, Department, Subjects: subjectIDs },
      { new: true }
    );
